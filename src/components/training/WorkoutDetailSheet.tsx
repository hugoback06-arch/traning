...
function SessionOnlyDetail({ session }: { session: TrainingPlanSession }) {
  const targetEntries = session.target_data ? Object.entries(session.target_data) : []
        ...
        <div className="grid grid-cols-2 gap-2">
          {targetEntries.map(([key, value]) => (
            <div key={key} className="rounded-lg bg-surface-muted p-2.5">
              <p className="text-xs text-ink-secondary">{key}</p>
              <p className="text-sm font-medium text-ink-primary">{String(value)}</p>
            </div>
          ))}
        </div>
      )}
